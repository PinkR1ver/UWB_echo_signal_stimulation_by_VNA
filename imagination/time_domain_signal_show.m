...
hLineEnv = cell(length(index), 1);

for i = 1:length(index)

    hLineEnv{i} = plot(t, signal(:,i), 'LineWidth', 1.5, 'DisplayName', ['Index = ', num2str(index{i})]);
    hold on;
...
prompt = index;
for i = 1:length(index)
    prompt{i} = ['Show Signal in Index = ', num2str(index{i}), ' and MC signal'];
end
...
for i = 1:length(index)
    index{i} = ['Show Signal in Index = ', num2str(index{i})];
end
...
for i = 1:length(index)
    prompt{i} = ['Hide Signal in Index = ', num2str(index{i})];
end
